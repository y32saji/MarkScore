...
export class DiagramDetector {
  static detectMusicDiagram(text: string): boolean {
  ...
  private static hasMusicalPatterns(text: string): boolean {
  ...
  static extractDiagramContent(text: string): string {
    const lines = text.split('\n')
    const musicKeywordIndex = lines.findIndex(line => 
      DIAGRAM_KEYWORDS.some(keyword => 
